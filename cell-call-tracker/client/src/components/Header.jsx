...
export default function Header({ effectiveUser, user, setView, enableSmart = false }) {
  const displayName = (u) => {
    if (!u) return ''
    return (
      admins[u.id] || (typeof window !== 'undefined' && localStorage.getItem(`rpName_${u.id}`)) || u.username.replace(/#\d+$/, '')
    )
  }
  ...
  {/* pipe removed from nav */}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        {effectiveUser && effectiveUser.admin && (
          <span style={{ fontSize: 12, color: 'var(--text-light)', opacity: .95, fontWeight: 600 }}>{displayName(effectiveUser)}</span>
        )}
